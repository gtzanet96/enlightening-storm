...
public class NPC_Dialogue_Friend_1 : MonoBehaviour
{
    ...
    //with this var we know when the friend's dialogue ended, so that if the player re-enters the collider box, it doesn't appear again (could also be done with talked_once var but let's differentiate)
    bool dialogue_done = false;
    public GameObject first_collider;
    public static bool pack_things = false;
    ...
    void Update()
    {
        if (CheckForChat_Friend_1.entrance_friend_1 == true)
        {
            if (dialogue_done == false)
            {
                ...
                if (!inChat)
                {
                    ...
                    {
                        //new text variable so we can assign new font size/type for greek language
    ...
    void loadDialogue1TopSubTree()
    {
        ...
        {
            topText_gr.GetComponent<Text>().text = "";
    ...
    public void Bot()
    {
        ...
        else if (inDialogueBotBotSubTree)
        {
            ...
            if (talked_once == false)
            {
                Score_Script.score = Score_Script.score - 150;
                points_1.text = "Score:" + (Score_Script.score).ToString();
                points_2.text = "Score:" + (Score_Script.score).ToString();
                false_sound.Play();
            }
